...
- **Repo**: https://github.com/vitucakes/vitucakes (hasta 2026-07-05 estaba en `patriciovallerino/vitucakes`; GitHub redirige el viejo)
- **Live**: https://vitucakes.github.io/vitucakes/ (la URL vieja `patriciovallerino.github.io/vitucakes` dejó de existir con la transferencia)
...
- **Cuenta GitHub**: el repo es de **`vitucakes`** (mail `pvallerino@gmail.com`). El `gh` de la máquina de Patricio sigue autenticado como `patriciovallerino` (colaborador con write). Commits SIEMPRE con `pvallerino@gmail.com`.

## Cómo levantar

```bash
git clone https://github.com/vitucakes/vitucakes
cd vitucakes
npm install
npm run dev
```
...
// Receta (mostrada como "Producto" en UI)
{
  ...
  precioFijo: number | null,     // precio que el user cobra en la práctica. SOLO anotación: NO reemplaza al precio sugerido (costo × MARGEN), que sigue mandando en ventas, gestión y competencia
...
>
> ⚠️ **ÚNICA EXCEPCIÓN (acordada con el user el 2026-09-25):** El Granate puede sugerir **bajar** un precio **solo si el precio actual vino de Día** (`ins.fuentePrecio === 'Día'`) — flag `baja` en la sugerencia, card en verde "↓ X%" con aviso de revisar el producto, y el user confirma uno por uno (nada se aplica solo). Compra / A mano / Excel (sin `fuentePrecio`) siguen protegidos. Ver "Último estado (2026-09-25)". **No revertir: es a propósito.**
...
## Último estado (2026-09-25)

- 🐞 **"No toma la info de El Granate" — diagnóstico y fix.** El cron de El Granate funcionaba bien (34 insumos, 0 errores) y los 34 matcheaban por nombre, pero la app **descartaba los 34** por la regla de oro: en todos, el precio actual era ≥ al de El Granate. Causa raíz: **El Granate estuvo caído dos veces** (0 items entre 2026-05-12 y 06-01, y otra vez la semana del 06-16). En esas ventanas `cubiertosPorGranate` quedaba vacío y **Día ofrecía TODO**; se aplicaron precios de Día, que son **paquetitos minoristas** (coco 25 g, nuez 100 g, chocolate 100 g → 2-3× más caros por gramo) y a veces **productos equivocados** (para "Gelatina Sin Sabor" Día había matcheado "Gelatina Reducida **Sabor Frambuesa** 20 Gr"). Al volver El Granate, "nunca bajar" los dejó trabados arriba para siempre. Eran **12 insumos con `fuentePrecio: 'Día'`**: Chocolate (33,63→11 /g, −67%), Miel (−67%), Esencia de vainilla (−60%), Azúcar impalpable (−62%), Gelatina Sin Sabor (−61%), Coco rayado (−60%), Nuez (−59%), Polvo de hornear (−57%), Margarina (−51%), Manteca (−25%), Levadura (−10%), Dulce de leche (−9%).
  - **Decisión del user:** El Granate corrige a Día (excepción documentada en la regla de oro y en CLAUDE.md). Implementado en `sugerencias` de `ActualizarPreciosPage` (`corrigeDia = esGranate && baja && ins.fuentePrecio === 'Día'`); orden: primero las que suben, después las bajas (mayor baja arriba). **No se aplicó ninguna:** quedan como sugerencias para que el user confirme mirando el producto matcheado (ojo: "Chocolate" matchea *Chocolate Alpino Pins Con Leche* y "Margarina" matchea *Margarina Hojaldre Premium 5 Kg* — si Vitu usa otro tipo, no aplicar).
  - Quedan protegidos y **no** aparecen aunque El Granate sea más barato: los de **Compra** (Fécula de Mandioca 7 vs 3,5; Mix frutos secos 43,6 vs 22) y los **sin fuente** / Excel (Azúcar 1,4 vs 1,1; Cacao 36,1 vs 32; **Bicarbonato 16,64 vs 5, −70%**; Leche condensada; Nutella). Si alguno está mal, se corrige a mano en el insumo.
- 🐞 **Bug aparte — el cache del scrape manual tapaba al cron para siempre**: `ActualizarPreciosPage` leía `localStorage[vitucakes_precios_sugeridos_cache]` y si existía **no volvía a pedir el JSON del cron nunca más** en ese dispositivo (bastaba tocar "Actualizar manualmente" una vez). Ahora se muestran ambos y **gana el más nuevo por `generadoEn`**; los fetch van con `cache: 'no-store'`; y un scrape manual que vuelve **vacío** (proxy CORS caído, web cambiada) ya **no pisa** el cache (antes escondía El Granate para siempre).
- 🐞 **"Compré en El Granate y no veo impacto" (2026-09-25)** — dos problemas en las 3 compras del día:
  1. **Una compra más barata no bajaba el costo** (`aplicarCompraAInsumos` solo subía). Decisión del user: *el costo pasa a ser lo que pagaste, aunque sea más barato*. Ahora el modal de guardado de `CompraEditSheet` lista las líneas que **suben (↑) o bajan (↓)**, y `aplicarCompraAInsumos` baja **solo** con `actualizaPrecio: true` explícito (compras viejas sin flag nunca bajan; `false` = no tocar).
  2. **Error de carga que disparó costos**: Oreos (117 g por $10.200 → $87/g), Chocolinas (250 g por $11.400 → $45,6/g) y Galletitas Lincoln (153 g por $6.400 → $41,8/g) tenían el peso de **UN paquete** con el total de varios, y el modal viejo los aplicó; Manteca tenía **25.000 g** en vez de 2.500 (sumó 22,5 kg de stock de más). Nuevo detector `precioSospechoso` en `utils/stock.js` (umbrales **asimétricos**: ≥2,5× sube o ≤0,2× baja — pasar de súper a distribuidora da 0,3-0,5× y es legítimo): aviso rojo en la línea ("¿cargaste el peso de un solo paquete?" / "¿se escapó un cero?") y en el modal esas líneas arrancan **destildadas**. Testeado con 14 checks incl. los casos reales del día.
  - **Datos corregidos** (transacción sobre `compras` + `insumos`, stock ajustado por diferencia): Oreos 1.170 g (10 paq.) → $8,72/g, Chocolinas 1.500 g (6) → $7,60, Lincoln 612 g (4, **cantidad deducida del precio por paquete, a confirmar por el user**) → $10,46, Manteca 2.500 g → $11,20; y costo = lo pagado en Harina 0000 $1,20, Queso Crema $6,50, Nuez $19, Dulce de leche $3,40, Chocolate $10 y (a pedido del user) Queso semiduro $8,29 — este último comprado en otro lado, no en El Granate.
- **Receta "Chipa 1/2 kg" → "Chipa 250 g" (dato, 2026-09-25)**: el user la cocinó ×4 y rindió 1,75 kg (→ 1 receta = 437,5 g), así que se escaló ×4/7 para que rinda 250 g y se renombró (decisión del user: un solo producto; para vender 1/2 kg se cargan 2 unidades). Nuevas cantidades: Fécula 257,14 g, Queso semiduro 142,86 g, Manteca 80 g, Leche 57,14 ml, Huevo 1,14 u, Sal 5,71 g; **Bolsa de madera queda en 1** (el packaging no se escala). Rinde sigue en "1 unidad" a propósito (con rinde 250 aparecería "Convertir a 1 g" y dividiría todo por 250). Costo total $6.953 → precio sugerido $20.860 (antes ~$36.400). Las ventas previas de "Chipa 1/2 kg" no cambian (cada venta guarda su snapshot).
- **Técnica para editar datos reales de forma segura** (usada arriba): script Node **20** (`~/.nvm/versions/node/v20.20.1/bin`, con 18 falla) con el SDK `firebase` del repo vía `createRequire`, login anónimo y **`runTransaction`** sobre el doc (`vitucakes/recetas`): lee, **verifica que el estado actual sea el esperado** (si alguien lo tocó, aborta), cambia solo ese item y escribe atómico con `ignoreUndefinedProperties`. Correr primero en modo prueba (sin `--aplicar`) y después verificar por REST.
- Script de diagnóstico útil (scratchpad de la sesión, no committeado): cruzar `public/precios_sugeridos.json` contra los insumos de Firestore por REST y listar por item "sugiere / descarta por nunca bajar / sin match" + `fuentePrecio` actual. Es lo primero a correr si vuelve a pasar.

### Antes (2026-08-15)

- 🐞🐞🐞 **LA causa real de "no guarda los cambios" (por fin)**: `InsumosPage.handleSubmit` escribía `fuentePrecio: i.fuentePrecio`, y en los insumos que **no tienen** `fuentePrecio` (los que no muestran badge Día/El Granate/A mano, ej. **Huevo**) eso mandaba la clave en **`undefined`**. Firestore **rechaza el documento COMPLETO** si hay un solo `undefined`: `FirebaseError: setDoc() called with invalid data. Unsupported field value: undefined (found in document vitucakes/insumos)`. Resultado: la UI mostraba el cambio (optimista) y unos segundos después **volvía atrás** al llegar el snapshot del servidor con el valor viejo. Por eso fallaba con unos insumos sí y con otros no. Encontrado analizando el **video del user** (Huevo pasaba a 227 u y volvía solo a 66,78) + reproducción en el sitio publicado leyendo la consola. **Fix en 3 capas**: (1) `InsumosPage` ya no escribe la clave si no hay fuente; (2) `firebase.js` con **`ignoreUndefinedProperties: true`** como red de seguridad global (un `undefined` suelto nunca más tumba un guardado entero); (3) `useSharedState.flush` envuelve `setDoc` en **try/catch** — tiraba SINCRÓNICAMENTE con datos inválidos, el `.catch()` no lo agarraba y quedaba "Uncaught" (fallo silencioso), y ahora además **avisa al user con un alert** si una escritura falla.
  - ⚠️ **Lección**: nunca escribir claves con `undefined` a Firestore, y que un fallo de guardado **nunca** sea silencioso.
  - Nota: durante la verificación, el stock de **Huevo quedó en 227 u** (el valor que el user intentaba cargar en su video).
- **Conteo de stock cargado (2026-08-15)**: el user pasó un inventario a mano y se cargaron 20 insumos por "Revisar stock" (Huevo 227 u, Harina 0000 3500, Azúcar impalpable 4500, Chocolate 1000, Fécula de Mandioca 2300, Coco rayado 950, Maicena 2120, Cacao alcalino 994, Nuez 200, Polvo de hornear 400, Lata de durazno 1 u, Chocolinas 1500, Oreos 1180, Vainillas 2 u, Dulce de leche 18000, Manteca 6875, Crema de leche 4500, Queso Crema 1500, Azúcar 18000, Leche 500 ml). Criterios de match acordados con el user: "Coco"→Coco rayado, "Almidón de maíz"→Maicena, "Cacao alcalina"→Cacao alcalino (existe además "Cacao" común, sin tocar), "Crema"→Crema de leche (g). Chocolinas/Oreos van en **gramos** aunque el user cuente paquetes; Lata de durazno y Vainillas en **unidades** (1 lata, 2 paquetes). Antes del conteo había **stock negativo** en Nuez (−626 g) y Leche (−100 ml) — ventas cargadas sin su compra.

### Antes (2026-07-24)

- **Precio fijo por producto** (`receta.precioFijo`) — campo opcional "Precio fijo que cobrás ($)" en el form del producto. Es **solo una anotación**: se muestra chiquito y en gris **debajo** de la card del precio sugerido, con el % de diferencia ("+11% vs sugerido"). **El sugerido (costo × 3) SIEMPRE prevalece**: es el que usan las ventas, Gestión y la comparación con la competencia. Vacío = no se muestra nada.
- 🐞🐞 **BUG GRAVE ARREGLADO — se perdían cambios (la causa REAL de "no guarda")**. `useSharedState` ignoraba el snapshot cuyo JSON coincidía con `lastWrittenJson` (la ÚLTIMA escritura). Con dos escrituras seguidas (ej: guardar stock y enseguida abrir un insumo, que suma `usos`) pasaba esto: llegaba el eco de la escritura VIEJA → como no coincidía con la última, se aplicaba → **la UI y `valueRef` volvían al valor viejo**; después llegaba el eco de la nueva → coincidía con `lastWrittenJson` → se ignoraba, dejando la pantalla en el valor viejo (parece "no se guardó"). Y lo peor: la edición siguiente partía de ese `valueRef` stale y **pisaba el cambio en Firestore**. Reproducido con una simulación de la lógica (scratchpad `sim-echo.mjs`). **Fix**: se eliminó `lastWrittenJson`; ahora se cuentan las escrituras propias sin confirmar (`pending` + `enVuelo`, este último con `.finally()` del `setDoc`) y **mientras haya alguna, los snapshots se ignoran** (lo local manda); cuando se confirman, el siguiente snapshot trae todo. Los cambios de otro dispositivo se siguen reflejando cuando estamos quietos. Verificado en la app real: guardar stock + segunda escritura inmediata → el valor sobrevive en Firestore y en pantalla.
...
- **MIGRACIÓN DE CUENTA GITHUB: `patriciovallerino` → `vitucakes` — ✅ COMPLETADA (2026-07-05)**. Cuenta nueva `vitucakes` (mail `pvallerino@gmail.com`). Repo transferido vía API y aceptado; `patriciovallerino` quedó como **colaborador con push** (el `gh` de la máquina de Patricio sigue autenticado ahí, sin re-auth). Verificado post-transferencia: los 4 workflows **activos** (deploy + 3 crons), Pages con `build_type: workflow` y la **URL nueva `https://vitucakes.github.io/vitucakes/` deployando OK** (la vieja murió). Remotes actualizados a `https://github.com/vitucakes/vitucakes.git` y `user.email = pvallerino@gmail.com` en: worktree + repo de Documents (comparten config) y carpeta de Drive. Referencias hardcodeadas actualizadas (REPO_ISSUE_URL de AgregarCompetidoraPage, README, CLAUDE.md, RECONSTRUIR, este archivo). **Vitu: actualizar el marcador/ícono a la URL nueva y re-meter el PIN** (localStorage es por dominio). Los datos no se tocaron (viven en Firebase). **Independencia de la empresa verificada**: repo y Pages en cuenta `vitucakes` (personal), copia del repo + backups de Firestore en el Drive del gmail personal (backup completo de 2026-07-05 confirmado), y el proyecto Firebase con dos Propietarios personales (`patriciovallerino@gmail.com` + `pvallerino@gmail.com`). Lo único atado a la máquina: el launchd del backup semanal (re-armable con RECONSTRUIR/HANDOFF) y el `gh` autenticado como `patriciovallerino` (el repo NO depende de esa cuenta, es solo colaboradora). **Mail primario de `patriciovallerino` cambiado a `pmvallerino@gmail.com` (personal) el 2026-07-05, verificado vía API** — el de Lemon quedó como secundario a propósito (posible SSO/dominio verificado de la org de Lemon; borrarlo recién al dejar la empresa). Con esto el squash ya no filtraría Lemon, pero se sigue mergeando por push directo para que los commits salgan con `pvallerino@gmail.com`.
...
- **Detalle de venta al tocarla** — en Ventas, tocar una card abre un BottomSheet con el detalle: items con precio unitario y subtotal, total, **"Stock descontado"** (el `consumo` guardado en el record, con nombre/unidad del insumo; borrado → "Insumo borrado") y botones Editar/Borrar. Los ✏️/🗑️ de la card usan `stopPropagation` para no abrir el detalle.
...
- **Firebase** (proyecto `vitucakes`): dos **Propietarios personales**, verificado con captura de "Usuarios y permisos" el 2026-07-05 — `patriciovallerino@gmail.com` (creador) y `pvallerino@gmail.com` (agregado ese día). Nada del proyecto depende de cuentas de Lemon.
...
- Reglas de workflow agente (lecciones aprendidas):
  ...
  - ⚠️ **Cómo mergear SIN filtrar el mail (repo público, lección 2026-06-08):** commiteá SIEMPRE con `pvallerino@gmail.com` (hasta 2026-07-05 era `patriciovallerino@gmail.com`; NUNCA el de Lemon). **NO uses `gh pr merge --squash`**: el squash estampa el commit en `main` con el mail PRIMARIO de la cuenta de GitHub autenticada en `gh` (si es `patriciovallerino`, su primario es el de Lemon) y pisa tu config local — "Keep my email private" NO lo evita. **Forma correcta: push directo a `main` con el commit en gmail** → `git commit` (user.email gmail) → `git rebase origin/main` si hace falta → `git push origin <rama>:main` → `git fetch && git reset --hard origin/main`. Podés abrir un PR para registro; GitHub lo marca "merged" al detectar el commit en main. (El 2026-06-08 se reescribió el historial para sacar el mail de Lemon que el squash venía filtrando.)

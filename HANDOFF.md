...
- **MIGRACIÓN DE CUENTA GITHUB: `patriciovallerino` → `vitucakes`** (en curso al cierre de esta sesión). Cuenta nueva `vitucakes` (mail `pvallerino@gmail.com`, creada 2026-07-05). Transferencia del repo iniciada vía API; **pendiente**: (1) aceptar la transferencia desde la cuenta `vitucakes` (mail/notificación, vence ~1 día), (2) invitar a `patriciovallerino` como colaborador (Settings → Collaborators) para que la máquina de Patricio siga pusheando sin re-autenticar `gh`. Después de aceptar, verificar: Pages activo (Settings → Pages → Source: GitHub Actions) y URL nueva `vitucakes.github.io/vitucakes` deployando; los 3 crons semanales habilitados (`gh workflow list` — la transferencia a veces los desactiva); actualizar remotes locales (`git remote set-url origin https://github.com/vitucakes/vitucakes.git`) en la carpeta de Drive y la de Documents (GitHub redirige el viejo igual). **Vitu tiene que actualizar el marcador** (la URL vieja muere) y re-meter el PIN (localStorage por dominio). Los commits pasan a firmarse con `pvallerino@gmail.com` (config ya cambiada); referencias hardcodeadas actualizadas en este commit (REPO_ISSUE_URL de AgregarCompetidoraPage, README, CLAUDE.md, RECONSTRUIR, este archivo). Los datos NO se tocan (viven en Firebase).
